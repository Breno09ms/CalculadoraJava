...
public class InterfaceCalc extends javax.swing.JFrame {
    ...
    if (!aguardandoNovoNumero) {
        if (ultimaOperacao != null) {
            valorAnterior = calcControladora.RealizarOperacao(ultimaOperacao, valorAnterior, valorAtual);
        } else {
            valorAnterior = valorAtual;
        }
        tfValor.setText(DoubleToString(valorAnterior)); // Mostra o valor acumulado
    }
    ...
    if (aguardandoNovoNumero || tfValor.getText().equals("0,00")) {
        tfValor.setText(caractere);
        aguardandoNovoNumero = false;
    } else {
        if (caractere.equals(",") && tfValor.getText().contains(",")) {
            // ignora duplicata de vírgula
        } else {
